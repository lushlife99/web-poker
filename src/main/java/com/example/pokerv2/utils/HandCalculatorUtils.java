...
public class HandCalculatorUtils {
    ...
    public static GameResultDto calculateValue(List<Integer> cards) {
        ...
        if (value != 1) {
            ...
            if (value != -1) {
                handValue = 80000000000L + value;
            } else {
                value = fourOfAKindValue(cards, jokBoList);

                if (value != -1) {
                    handValue = 70000000000L + value;
                } else {
                    value = fullHouseValue(cards, jokBoList);

                    if (value != -1) {
                        handValue = 60000000000L + value;
                    } else {
                        value = flushValue(cards, jokBoList);

                        if (value != -1) {
                            handValue = 50000000000L + value;
                        } else {
                            value = straightValue(cards, jokBoList);

                            if (value != -1) {
                                handValue = 40000000000L + value;
                            } else {
                                value = threeOfAKindValue(cards, jokBoList);

                                if (value != -1) {
                                    handValue = 30000000000L + value;
                                } else {
                                    value = twoPairValue(cards, jokBoList);

                                    if (value != -1) {
                                        handValue = 20000000000L + value;
                                    } else {
                                        value = onePairValue(cards, jokBoList);

                                        if (value != -1) {
                                            handValue = 10000000000L + value;
                                        } else {
                                            handValue = highCardValue(cards, jokBoList);
                                        }
                                    }
                                }
                            }
